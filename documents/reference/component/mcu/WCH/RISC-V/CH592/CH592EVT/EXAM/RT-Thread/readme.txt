本工程为RT-Thread-Nano在Qingke V4C RISC-V内核的CH592/1上的移植。
...
需要注意以下几点：
	...
	11.CH592的蓝牙会采用类似于RTOS操作，在中断中切换到另一个运行环境等待蓝牙事件完成，所以如果使用蓝牙，则必须在ble_task_scheduler.S中，添加调用其他函数用来停止RT-Thread操作系统，防止在蓝牙内部任务中被切换。

	12.CH592不可以使用原core_riscv.h中的 __enable_irq 和 __disable_irq 函数，使用rt-thread中提供的 rt_hw_interrupt_disable 和 rt_hw_interrupt_enable。
