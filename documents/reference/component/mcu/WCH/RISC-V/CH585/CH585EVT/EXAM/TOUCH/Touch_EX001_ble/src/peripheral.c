...
void Peripheral_Init()
{
    ...
    // Setup the GAP Peripheral Role Profile
    {
        ...
        advState = initial_advertising_enable;
...
void peripheralChar2Notify(uint8_t *pValue, uint16_t len)
{
    ...
    if (simpleProfile_Notify2( peripheralConnList.connHandle, &noti ) != SUCCESS)
    {
...
void peripheralChar3Notify(uint8_t *pValue, uint16_t len)
{
    attHandleValueNoti_t noti;
    noti.len = min( (peripheral_Mtu - 3), len );
    noti.pValue = GATT_bm_alloc( peripheralConnList.connHandle, ATT_HANDLE_VALUE_NOTI, noti.len, NULL, 0 );
    tmos_memcpy( noti.pValue, pValue, noti.len );
    if (simpleProfile_Notify3( peripheralConnList.connHandle, &noti ) != SUCCESS)
    {
...
void peripheralChar4Notify(uint8_t *pValue, uint16_t len)
{
    ...
    noti.pValue = GATT_bm_alloc( peripheralConnList.connHandle, ATT_HANDLE_VALUE_NOTI, noti.len, NULL, 0 );
    tmos_memcpy( noti.pValue, pValue, noti.len );
    if (simpleProfile_Notify4( peripheralConnList.connHandle, &noti ) != SUCCESS)
    {
