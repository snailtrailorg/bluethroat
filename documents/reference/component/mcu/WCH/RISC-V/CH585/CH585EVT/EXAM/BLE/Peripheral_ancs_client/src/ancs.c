...
{
    if(!(ancs_c.state & (1<<ANCS_BT_CONNECTED))){
    ...
    if(!(ancs_c.state& (1<<ANCS_BT_DISCOVERY_COMPLETE))){
...
static void Peripheral_LinkEstablished(gapRoleEvent_t *pEvent)
{
    ...
    else
    {
        ...
        value_bit_set(&ancs_c.state, ANCS_BT_CONNECTED,TRUE);
      ...
      case DISC_ANCS_CCCD:
        {
              ...
              (pMsg->method == ATT_ERROR_RSP))
          {
            ...
            else
            {
              ...
              value_bit_set(&ancs_c.state, ANCS_BT_DISCOVERY_COMPLETE,TRUE);
              PRINT("discover complete\n");
              tmos_set_event(ancs_taskid, START_SUBSCRIPE_NOTIDATA_EVT);
              tmos_start_task(ancs_taskid, START_SUBSCRIPE_NOTISRC_EVT, 300);
