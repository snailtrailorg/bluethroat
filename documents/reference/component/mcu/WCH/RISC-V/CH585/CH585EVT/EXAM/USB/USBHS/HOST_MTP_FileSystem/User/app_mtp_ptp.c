...
uint8_t MTP_SendData( uint8_t usb_port, uint8_t *pbuf, uint32_t len )
{
    ...
    while( len )
    {
        if( usb_port == DEF_USBFS_PORT_INDEX )
        {
            ...
            {
                R32_U2H_TX_LEN = HostCtl[ index ].Interface[ 0 ].OutEndpSize[ 0 ];
...
uint8_t MTP_RecvData( uint8_t usb_port, uint8_t *pbuf, uint32_t *plen )
{
    ...
    if( usb_port == DEF_USBFS_PORT_INDEX )
    {
        s = USBHSH_Transact( USB_PID_IN  | (HostCtl[ index ].Interface[ 0 ].InEndpAddr[ 0 ] << 4 ),
                             HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ], 20000 );

        if( s == ERR_SUCCESS )
        {
            HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ] ^= USBHS_UH_T_TOG_DATA1;
...
uint8_t USBH_PTP_GetStorageIds( uint8_t usb_port )
{
    ...
    PRINT( "PTP_StorageIDs[ usb_port ].n: %02x\r\n", PTP_StorageIDs[ usb_port ].n );
    ...
    switch( datatype )
    {
        ...
        case PTP_DTC_INT32:
            ...
            *offset += 4;
            break;
        case PTP_DTC_UINT32:
            PTP_PropertyValue[ usb_port ].uint32_t = LE32( &( data[ *offset ] ) );
            *offset += 4;
            break;
        case PTP_DTC_INT64:
...
void MTP_Init_Process( uint8_t usb_port )
{
    ...
    if( PTP_StorageIDs[ usb_port ].n ) // Indicates that the device is mounted
    {
        ...
        for( i = 0; i != total_num; i++ )
        {
            ...
            PRINT( "obj_info.ObjectFormat = %02x\r\n", PTP_ObjectInfo[ usb_port ].ObjectFormat );
            PRINT( "obj_info.FileName:" );
            for( s = 0; s != PTP_ObjectInfo[ usb_port ].FileName_len; s++ )
            {
                PRINT( "%02x ", PTP_ObjectInfo[ usb_port ].Filename[ s ] );
            }
            PRINT( "\r\n" );
            PRINT( "obj_info.Data_Creat:" );
            for( s = 0; s != PTP_ObjectInfo[ usb_port ].FileName_len; s++ )
            {
                PRINT( "%02x ", PTP_ObjectInfo[ usb_port ].Data_Creat_Time[ s ] );
            }
            PRINT( "\r\n" );
            PRINT( "obj_info.Data_Modified:" );
            for( s = 0; s != PTP_ObjectInfo[ usb_port ].FileName_len; s++ )
            {
                PRINT( "%02x ", PTP_ObjectInfo[ usb_port ].Data_Modified_Time[ s ] );
            }
            PRINT( "\r\n" );

            USBH_MTP_GetObjectSize( usb_port, MTP_Para[ usb_port ].mtp_handle.Handle_num );
            PRINT( "obj_info.File_Size = %08lx\r\n", (uint32_t)PTP_ObjectInfo[ usb_port ].File_Size );
        }
