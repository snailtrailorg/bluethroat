...
__attribute__((aligned(4))) static uint32_t gs_nfca_soft_pcd_data_buf[NFCA_PCD_DATA_BUF_SIZE];
 ...
 * @return  none
 */
void nfca_soft_pcd_init(void)
{
    nfca_soft_pcd_config_t cfg;
    uint8_t res;
...
//    R32_PIN_IN_DIS |= GPIO_Pin_9;
    ...
    cfg.data_buf = gs_nfca_soft_pcd_data_buf;
    ...
    /* 默认全部设置为超时时间 */
    for(uint16_t i = 0; i < NFCA_PCD_DATA_BUF_SIZE; i++)
    {
        gs_nfca_soft_pcd_data_buf[i] = 0x2000000;
    }

    /* 将数据区指针传入给NFC库内BUFFER指针 */
    res = nfca_soft_pcd_lib_init(&cfg);
    if(res)
    {
        PRINT("nfca soft pcd lib init error\n");
        while(1);
    }
#if NFCA_PCD_USE_NFC_CTR_PIN
    nfca_pcd_ctr_init();
...
/*********************************************************************
 * @fn      nfca_soft_pcd_wait_communicate_end
 *
 ...
 */
nfca_pcd_controller_state_t nfca_soft_pcd_wait_communicate_end(void)
{
    ...
    while (1)
    {
        status = nfca_soft_pcd_get_communicate_status();
        if (status != 0)
        {
            break;
        }

        if (overtimes > (NFCA_PCD_WAIT_MAX_MS * 2))
        {
            /* 软件超时时间， */
            break;
        }
    ...
    g_nfca_pcd_recv_buf_len = nfca_soft_pcd_get_recv_data_len();
    g_nfca_pcd_recv_bits = nfca_soft_pcd_get_recv_bits();
...
{
    NFCSoftPCD_IRQLibHandler();
}

/*******************************************************************************
 * @fn              TMR0_IRQHandler
 *
 * @brief           nfc-a 软件解码接收中断处理函数
 *
 * @param           None.
 *
 * @return          None.
