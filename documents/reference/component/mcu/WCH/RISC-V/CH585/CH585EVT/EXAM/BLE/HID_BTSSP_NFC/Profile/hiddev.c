...
void hidDevoobCB( uint8_t *deviceAddr, uint16_t connHandle, uint8_t* r_local, uint8_t *c_local);
/*********************************************************************
 * PROFILE CALLBACKS
 */
...
// Bond Manager Callbacks hidDevoobCB
...
void hidDevoobCB( uint8_t *deviceAddr, uint16_t connHandle, uint8_t* r_local, uint8_t *c_local)
{
    uint8_t  c_peer[16];
    uint8_t  r_peer[16];
    GAPBondMgr_OobRsp( connHandle, 0 , r_peer, c_peer );
}
